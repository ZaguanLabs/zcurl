...
.PHONY: all test check benchmark memcheck ubsan clean
all: build/zcurl.so
...
build/zcurl.so build/ubsan/zcurl.so: src/zcurl.c src/websocket.c src/http_async.c src/http_headers.c $(ZSH_HEADERS) Makefile
...
clean:
	rm -f build/zcurl.so build/zcurl.so.tmp build/ubsan/zcurl.so build/ubsan/zcurl.so.tmp
