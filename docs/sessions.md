# Named synchronous HTTP sessions (0.17.0-dev)

Named sessions give synchronous callers independent libcurl connection pools.
...
Requests without `--session` retain the existing default pool. Named sessions
are synchronous only; concurrent `http submit` and WebSocket operations reject
`--session`. Concurrent requests retain their existing shared pool, and each
WebSocket keeps its own connection. Their handle namespaces remain independent.
...
| `zcurl session reset NAME` | Close that session's connections and destroy its easy/multi handles, retaining the name and creation order |
...
status 2. Allocation failures return 27. Management errors print a diagnostic.
...
Requests keep the ordinary 13-field synchronous snapshot shape and accept the
existing HTTP options, including proxy controls, compression and file I/O.
...
Each session owns a separate easy/multi pair with no libcurl share handle between
sessions. Connections and libcurl DNS/TLS caches can be reused inside that pair.
...
use `session reset` or a distinct name when a separate pool is needed. Cookies,
named concurrent pools and per-session configuration are outside this milestone.
