...
def test_https_proxy(env, plain, temp, run, setup):
    ...
    with fixture(env, context) as (server, proxy_env):
        ...
        ''', 1)
        # Session trust defaults remain independent and are copied at submit.
        check('''
            typeset -A session_state
            zcurl session create trusted
            zcurl session configure trusted --cacert "$ZCURL_TEST_CA" --proxy-cacert "$PROXY_CA"
            zcurl --session trusted "$ZCURL_TEST_HTTPS/tiny"
            check "$zcurl_body" $'ok\\n'
            zcurl session info trusted -r session_state
            check "$session_state[cacert]" "$ZCURL_TEST_CA"
            check "$session_state[proxy_cacert]" "$PROXY_CA"
            zcurl http submit retained --session trusted "$ZCURL_TEST_HTTPS/tiny"
            zcurl session configure trusted --defaults
            zcurl http wait retained
            zcurl http collect retained
            check "$zcurl_body" $'ok\\n'
            if zcurl --session trusted "$ZCURL_TEST_HTTP/tiny"; then exit 1; else check $? 60; fi
            zcurl session configure trusted --proxy-cacert "$PROXY_CA"
            if zcurl --session trusted "$ZCURL_TEST_HTTPS/tiny"; then exit 1; else check $? 60; fi
            zcurl session configure trusted --cacert "$ZCURL_TEST_CA" --proxy-cacert ''
            if zcurl --session trusted "$ZCURL_TEST_HTTP/tiny"; then exit 1; else check $? 60; fi
            zcurl --proxy-cacert "$PROXY_CA" --session trusted "$ZCURL_TEST_HTTP/tiny"
            zcurl --session trusted --proxy-cacert "$PROXY_CA" "$ZCURL_TEST_HTTP/tiny"
            zcurl session configure trusted --proxy-cacert "$PROXY_CA"
            zcurl session reset trusted
            zcurl --session trusted "$ZCURL_TEST_HTTPS/tiny"
